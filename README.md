...
**AI Settlement Auditor** is a fintech application built around a simple principle: complete settlement transparency and automated, safe recovery.
...
Pre-defined, strict policy bounds allow the system to automatically trigger recovery actions for known, safe issues such as retrying a webhook or initiating a gateway refund sync.
...
```mermaid
flowchart TD
    S1[1. Ingest Razorpay Data] --> S2[2. Reconcile with Ledger]
    S2 --> S3[3. Detect Anomalies]
    S3 --> S4[4. AI Generates Explanation]
    S4 --> S5[5. Evaluate Recovery Rules]
    S5 --> S6[6. Human Gating / Approval]
    S6 --> S7[7. Execute and Audit Log]
```
...
flowchart TD
    ...
    D --> E{Expected equals Actual Ledger?}
...
flowchart TD
    ...
    F -->|Reject| H[Halt and Log Failure]
```
...
```bash
git clone https://github.com/riti2043/AI_settlement_auditor.git
