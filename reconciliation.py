...
class ReconciliationEngine:
    ...
    def run_reconciliation(self, session_id: str = "default"):
        """
        Runs the full reconciliation pipeline scoped to the given session_id.
        """
        conn = get_db()
        ...
        # ------------------------------------------------------------------
        # 1. CUSTOMER RISK TIERING (scoped to session)
        # ------------------------------------------------------------------
        cust_counts = (
            df[df['status'].isin(['Mismatched', 'Flagged'])]
            .groupby('customer_id')
            .size()
            .to_dict()
        )
        for _, row in df.iterrows():
            failures = cust_counts.get(row['customer_id'], 0)
            tier = "High" if failures >= 3 else ("Medium" if failures == 2 else "Low")
            cursor.execute(
                "UPDATE transactions SET risk_tier = ? WHERE id = ? AND session_id = ?",
                (tier, row['id'], session_id)
            )
        conn.commit()

        # ------------------------------------------------------------------
        # 2. DUPLICATE CHARGE DETECTOR (within session only)
        # ------------------------------------------------------------------
        ...
        # Reload updated dataframe for this session
        df = pd.read_sql_query(
            "SELECT * FROM transactions WHERE session_id = ?",
            conn,
            params=(session_id,)
        ...
        for _, row in df.iterrows():
                    ...
                    if abs(actual_gst - expected_gst) > 0.05:
                        cursor.execute('''
                            ...
                            SET tax_mismatch = 1, status = 'Mismatched',
                                ...
                                action_taken = 'Flagged: GST Discrepancy Detected (Expected Rs.' || ? || ' vs Actual Rs.' || ? || ')'
                            WHERE id = ? AND session_id = ?
                        ''', (expected_gst, actual_gst, tx_id, session_id))
                        conn.commit()
            ...
            if status == "Mismatched":
                    ...
                    if due_date:
                        try:
                            ...
                            if due_dt < datetime.now():
                                ...
                                cursor.execute('''
                                    UPDATE transactions
                                    SET status = 'BROKEN_PROMISE', action_taken = ?,
                                        escalation_step = 2, confidence_note = ?
                                    WHERE id = ? AND session_id = ?
                                ''', (action, confidence_note, tx_id, session_id))
                                cursor.execute('''
                                    INSERT INTO audit_log (session_id, transaction_id, action, details, is_live_api_call)
                                    VALUES (?, ?, ?, ?, 0)
                                ''', (session_id, tx_id, "BROKEN_PROMISE_ESCALATED",
                                      f"Overdue commitment date ({due_date}). Sent SMS reminder."))
                                conn.commit()
                                continue
                        except Exception:
                            pass
                ...
                # ------------------------------------------------------------------
                    ...
                    confidence_note = f"Action required: Execute live Razorpay Refund API for Rs.{amount:,.2f} within bounded gateway limits."
                    ...
                    if esc_step == 1:
                        ...
                        cursor.execute('''
                            UPDATE transactions
                            SET action_taken = ?, escalation_step = 2, confidence_note = ?
                            WHERE id = ? AND session_id = ?
                        ''', (action, confidence_note, tx_id, session_id))
                        cursor.execute('''
                            INSERT INTO audit_log (session_id, transaction_id, action, details, is_live_api_call)
                            VALUES (?, ?, ?, ?, 1)
                        ''', (session_id, tx_id, "ESCALATION_STEP_1",
                              "Attempted gateway retry for failed subscription."))
                        conn.commit()
                        continue
                    elif esc_step == 2:
                        cursor.execute('''
                            ...
                            SET status = 'Flagged', flagged = 1,
                                flag_reason = 'Subscription retry failed twice. Requested Rs.150 discount incentive requires review.',
                        ...
                        cursor.execute('''
                            INSERT INTO audit_log (session_id, transaction_id, action, details, is_live_api_call)
                            VALUES (?, ?, ?, ?, 0)
                        ''', (session_id, tx_id, "ESCALATION_STEP_2_GATE",
                              "Subscription retry failed twice. Escalated to Fair Incentive Gate."))
                        conn.commit()
                        continue
                ...
                if recovered:
                    ...
                    cursor.execute('''
                        ...
                        WHERE id = ? AND session_id = ?
                    ''', (new_status, action, fee_json, confidence_note,
                          1 if is_live else 0, datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                          tx_id, session_id))
                    cursor.execute('''
                        INSERT INTO audit_log (session_id, transaction_id, action, details, is_live_api_call)
                        VALUES (?, ?, ?, ?, ?)
                    ''', (session_id, tx_id, "AUTOMATED_RECOVERY_EXECUTED",
                          f"Agent executed: {action}", 1 if is_live else 0))
                    conn.commit()
    ...
    def execute_action(self, tx_id: str, cause: str, session_id: str = "default"):
        """
        Executes a bounded recovery action for the given transaction.
        ...
        if not row:
            conn.close()
            return {"success": False, "message": "Transaction not found or does not belong to this session."}
        ...
        if cause == "insufficient_funds":
            ...
            details = f"Created Razorpay payment link for Rs.{amount} split."
            merchant_msg = f"Payment of Rs.{amount} declined (insufficient funds). A split-payment link has been sent to the customer."
            customer_msg = f"Your payment of Rs.{amount} was declined. You can complete it in two installments using this link: https://rzp.io/i/mock_split_{tx_id}"
            is_live = True
            ...
            merchant_msg = f"Card blocked for Rs.{amount}. A new payment link for UPI/Netbanking has been sent."
            ...
            merchant_msg = f"CRITICAL: Bank flagged Rs.{amount} transaction as potential fraud. Do not retry. See risk dashboard."
            cursor.execute(
                "UPDATE transactions SET flagged = 1, flag_reason = 'Bank Fraud Flag' WHERE id = ? AND session_id = ?",
                (tx_id, session_id)
            )
            ...
            is_live = True
            import random as _r
            refund_ref = _r.randint(1000, 9999)
            merchant_msg = f"Stuck refund for Rs.{amount} has been re-triggered. Razorpay Refund ID: rfnd_test_{refund_ref}. Expected in 5-7 days."
            customer_msg = f"Your refund of Rs.{amount} has been processed. Reference ID: rfnd_test_{refund_ref}. Please allow 5-7 business days."
            cursor.execute(
                "UPDATE transactions SET status = 'Recovered' WHERE id = ? AND session_id = ?",
                (tx_id, session_id)
            )
            ...
            merchant_msg = f"Your settlement of Rs.{amount} is on hold. Action needed: upload PAN card at razorpay.com/dashboard -> Settings -> KYC."
        else:
        ...
        cursor.execute('''
            INSERT INTO audit_log (session_id, transaction_id, action, details, is_live_api_call)
            VALUES (?, ?, ?, ?, ?)
        ''', (session_id, tx_id, "EXECUTE_AGENT_ACTION", details, 1 if is_live else 0))
...
if __name__ == "__main__":
    ...
    summary = engine.run_reconciliation(session_id="test")
